...
public class EditPertenencia extends AppCompatActivity {

    EditText etNombre, etCategoria, etDimensiones, etUnidades, etPeso, etValor;
    CheckBox cbFragil;
    Button btnFecha, btnGuardar;
    LocalDate fechaSeleccionada = null; // Inicializo a null por si acaso
    Pertenencia pertenenciaOrigen;
    int posicion; // La posición del objeto en la lista de pertenencias
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Copio las referencias de AddPertenencia, total es lo mismo
        ...
        cbFragil = findViewById(R.id.checkBoxFragil);
        btnFecha = findViewById(R.id.btnFecha); // Hacer clic en el botón abre un DatePicker
        btnGuardar = findViewById(R.id.btnGuardar);

        // Ahora toca recibir los datos desde MainActivity
        pertenenciaOrigen = (Pertenencia) getIntent().getSerializableExtra("pertenencia");
    ...
    private void cargarDatosOriginales(){
        etNombre.setText(pertenenciaOrigen.getNombrePertencia());
        etCategoria.setText(pertenenciaOrigen.getCategoria());
        etUnidades.setText(String.valueOf(pertenenciaOrigen.getUnidades()));
        etPeso.setText(String.valueOf(pertenenciaOrigen.getPesoUnidad()));
        etDimensiones.setText(pertenenciaOrigen.getDimensiones());
        cbFragil.setChecked(pertenenciaOrigen.isFragil());

        // Aquí tomo los valores de los campos opcionales si es que están rellenos
        if(pertenenciaOrigen.getValorUnidad() > 0){
            etValor.setText(String.valueOf(pertenenciaOrigen.getValorUnidad()));
        }

        if(pertenenciaOrigen.getFechaCompra() != null){
            fechaSeleccionada = pertenenciaOrigen.getFechaCompra();
            btnFecha.setText("Fecha: "+ fechaSeleccionada.toString()); // todo poner el @string
        }
    }

    // Copio la función de mostrar el datePicker del AddPertenencia
    private void mostrarDatePicker(){
        ...
        DatePickerDialog dialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    btnFecha.setText("Fecha: " + fechaSeleccionada.toString()); // todo poner el @string aquí también
                },
                anio, mes, dia
        );
        dialog.show();
    ...
    private void guardarCambios(){
        ...
        resultIntent.putExtra("posicion", posicion); // Importante esto, necesito saber la posición en la listaPertenencias de este objeto para implementar los cambios.
